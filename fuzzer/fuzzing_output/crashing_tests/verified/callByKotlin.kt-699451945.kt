...
annotation data class OneDefault(val s: String = "OK")
...
return ctor.callBy(args.mapKeys((if (true) {
({entry -> ctor.(when {
true -> (parameters.single({it.name == entry.key}))
else -> (parameters.single({it.name == entry.key}))
})})
} else {
({entry -> ctor.(when {
true -> (parameters.single({it.name == entry.key}))
...
val t2 = create<OneNonDefault>(mapOf("s" to (try {
"OK"
}
finally {
"OK"
...
assertFails("KClass (not Class) instances should be passed as arguments")({(create)!!<TwoNonDefaults>(mapOf("klass" to String::class.java, "string" to "Fail"))})
