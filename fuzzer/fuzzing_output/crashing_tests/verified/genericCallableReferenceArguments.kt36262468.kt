...
inline fun <reified T, reified R> check(x: T, y: R, f: ((T) -> R), tType: String, rType: String): Unit {
assertEquals(tType, T::class.simpleName)
assertEquals(rType, R::class.simpleName)
}
inline fun <reified T, reified R> check(f: ((T) -> R), g: ((T) -> R), tType: String, rType: String): Unit {
assertEquals(tType, T::class.simpleName)
assertEquals(rType, (R)?::class.simpleName)
}
fun box(): String {
