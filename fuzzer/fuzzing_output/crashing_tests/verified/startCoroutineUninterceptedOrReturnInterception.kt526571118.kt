...
suspend fun suspendHere(): String = suspendCoroutineOrReturn({x -> x.resume("OK")
COROUTINE_SUSPENDED})
suspend fun suspendWithException(): String = suspendCoroutineOrReturn((if (true) {
({x -> x.resumeWithException(RuntimeException("OK"))
COROUTINE_SUSPENDED})
} else {
({x -> x.resumeWithException(RuntimeException("OK"))
COROUTINE_SUSPENDED})
}))
fun builder(shouldSuspend: Boolean, expectedCount: Int, c: (() -> String)): String {
...
fun box(): String {
if (builder(false, 0)({"OK"}) != "OK") {
return "fail 4"
}
if (builder(true, 1)({suspendHere()}) != "OK") {
return "fail 5"
}
if (builder(false, 0)({throw RuntimeException("OK")}) != "Exception: OK") {
return "fail 6"
}
if (builder(true, 1)({suspendWithException()}) != "Exception: OK") {
return "fail 7"
}
return "OK"
}